...
export default function OnboardingPage() {
  ...
  function goNext() {
    if (step === 2) saveStep("profile", { businessName, accentColor });
    else if (step === 3) saveStep("info", { firstName, email, phone });
    else if (step === 4) saveStep("hours", { businessHours });
    else if (step === 5) saveStep("services", { services: services.filter((s) => s.enabled) });
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-md mx-auto px-4 py-8 min-h-screen flex flex-col">
        ...
        {/* Sliding step container — all 6 steps always mounted */}
          ...
          >
                  ...
                  {DAYS.map((day) => {
                    ...
                    return (
                      <div key={day} className="flex items-center gap-3">
                        <button onClick={() => toggleDay(day)} className="w-11 h-6 rounded-full relative transition-colors duration-200 shrink-0" style={{ backgroundColor: isOpen ? accentColor : "rgba(255,255,255,0.06)" }}>
                          <div className="w-5 h-5 rounded-full bg-white absolute top-0.5 transition-all duration-200 shadow-sm" style={{ left: isOpen ? "22px" : "2px" }} />
                        </button>
                        <span className={`text-sm w-24 shrink-0 ${isOpen ? "text-white/70" : "text-white/20"}`}>{DAY_LABELS[day]}</span>
                        {isOpen ? (
                          <div className="flex items-center gap-2 flex-1">
                            <input type="time" value={businessHours[day]!.open} onChange={(e) => updateHours(day, "open", e.target.value)} className="text-xs bg-white/[0.03] border border-white/[0.08] rounded-lg px-2 py-1.5 text-white/60 w-24 focus:outline-none" />
                            <span className="text-white/15 text-xs">–</span>
                            <input type="time" value={businessHours[day]!.close} onChange={(e) => updateHours(day, "close", e.target.value)} className="text-xs bg-white/[0.03] border border-white/[0.08] rounded-lg px-2 py-1.5 text-white/60 w-24 focus:outline-none" />
                          </div>
                        ) : (
            ...
            <div className="w-full flex-shrink-0 overflow-y-auto px-1">
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  {services.map((svc, i) => (
                    <div key={i} className={`border rounded-2xl p-4 transition-all duration-200 ${svc.enabled ? "bg-white/[0.04] border-white/[0.08]" : "bg-white/[0.01] border-white/[0.03] opacity-50"}`}>
                        ...
                        <div className="flex gap-3 ml-8">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <input type="number" value={svc.price} onChange={(e) => updateServiceField(i, "price", e.target.value)} className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg px-2 py-1.5 text-sm text-white/70 focus:outline-none" />
                            </div>
                          </div>
                          <div className="flex-1">
                            <label className="text-[10px] text-white/25 uppercase tracking-wider block mb-1">Minutes</label>
                            <input type="number" value={svc.duration} onChange={(e) => updateServiceField(i, "duration", e.target.value)} className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg px-2 py-1.5 text-sm text-white/70 focus:outline-none" />
                          </div>
                        </div>
                      )}
                    </div>
        ...
        <div className="mt-8 pb-4">
          <div className="flex gap-3">
            {step > 1 && step < 6 && (
              <button onClick={goBack} className="flex-none px-4 py-3 rounded-xl text-sm font-medium text-white/40 hover:text-white/60 bg-white/[0.04] hover:bg-white/[0.06] border border-white/[0.06] transition-all duration-200">Back</button>
            )}
            {step === 1 && (
